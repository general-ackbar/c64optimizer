...
The resulting image will be saved as a new image that is already ready to be converted to `koala` or `prg` by another tool, such as png2prg by Burglar.
...
| Option | Description |
| --- | --- |
| `-i (file)` | Name of the input file. This must be a valid bitmap. |
...
| `-d` | Dither output using the Floyd-Steinberg algorithm. |
